Theme.ShippingCalculator = (function($, Countries, Money) {
  /**
  ...
  Shopify.Cart.ShippingCalculator = (function() {  
    ...
    var _onCartShippingRatesUpdate = function(rates, shipping_address) {
      ...
      if (rates.length) {
        ...
        for (var i=0; i<rates.length; i++) {
          rates[i].formatted_price = _formatRate(rates[i].price);
        }
      }
    ...
    _init = function() {
      ...
      jQuery('.shipping-calculator-form').submit(function(event) {
  ...
  // Update total price on updating shipping estimate
  (function(){
    $('#shipping-calculator').on('change', '.shipping-cost', function(){
      var totalPrice = $('.total-price');

      var total = totalPrice.filter('.estimated-total-price').attr('data-price');
      total = parseInt(total.replace(/\D/g, ""));
      var shipping = parseInt(this.value.replace(/\D/g, ""));
      totalPrice.hide().fadeIn('fast').text(Money.format(total + shipping));

    });
  })();
  
  return Shopify.Cart.ShippingCalculator;
})(jQuery, Theme.Countries, Theme.Money);